...
После установки удобнее всего запускать единое меню:

```powershell
python app.py
```

В меню доступны подготовка кадров, разметка, обучение, дообучение и обработка видео.
...
Аргументы после `infer.py`: сначала путь к исходному видео, затем путь к файлу модели. Результат — MP4, где вне дороги красный цвет, края дороги жёлтые, а центральная безопасная зона зелёная. Белые линии показывают границы зон, синяя — внешний контур дороги.

Центр не вычисляется заново независимо в каждой строке. Коридорный трекер запускается в разрешении модели, запоминает траекторию от точки автомобиля, переносит её между соседними кадрами по оптическому потоку и выбирает новую траекторию только с учётом предыдущей. Поэтому при частично видимой, расширяющейся или похожей на поле дороге центр не должен внезапно развернуться поперёк дороги. Отдельное ответвление принимается только после нескольких последовательных кадров и получает продолжение от общей центральной траектории.
...
### Проверка геометрии зон

В проекте есть автоматические сценарии для перекрёстка и Т-образной развилки:

```powershell
python -m unittest tests/test_road_geometry.py tests/test_corridor_tracker.py
```
